...
3. install gcc and gdb in wsl (see makefile for install path, under usr/local/)
3. in wsl: `export DISPLAY=$(route.exe print | grep 0.0.0.0 | head -1 | awk '{print $4}'):0.0`
   - if you have wsl1 (like i do), instead use `export DISPLAY=:0`, since wsl1 runs on the same IP addresses as windows (good idea to add this to your .bashrc)
4. `make run`
5. to clean .o and .bin files, run `make clean`


### on linux
i assume its a lot easier given the patchwork necessary on windows thru wsl.
1. install qemu, gcc, and gdb (see make file for gcc/gdb install path under usr/local/)
3. `make run`
